 ...
 * Behavior of Diffuse2dImpl includes both diffusion and "evaporation" of
 * whatever the values represent, at rates we can specify.
 * <p>
 * The class HeatSpace specializes Diffuse2dImpl; we use the integer values to
 * represent heat (which Heatbugs produce as well as seek).
 * <p>
 * We use <tt>Diffuse2dImpl.getValueAtX$Y()</tt> and
 * <tt>Diffuse2dImpl.putValue$atX$Y()</tt> to access the integer values.
 * <p>
...
public class HeatSpace extends Diffuse2dImpl {
	...
	public double getDiscardedHeat() {
		return _discardedHeat;
	}
	...
	public HeatSpace(Zone aZone, int worldXSize, int worldYSize,
			double diffusionConstant, double evaporationRate,
	...
	public Object addHeat(int moreHeat, int x, int y) {
		...
		if (heatHere + moreHeat <= MAX_HEAT) // would add be too big?
			...
			if (_printDiagnostics >= 21)
				System.out.println("In HeatSpace.addHeat() at (" + x + "," + y
						+ "), I discarded heat " + heatHere + " + " + moreHeat
						+ " - " + MAX_HEAT + " = " + discardedHeat + ".");
		}

		putValue$atX$Y(heatHere, x, y); // set the heat
		// Monitor the heat at an arbitrary cell (2, 2) (HeatbugModelSwarm
		// monitors the same cell):
		if (_printDiagnostics >= 10 && x == 2 && y == 2) {
			System.out.println("In HeatSpace.addHeat(), heat " + moreHeat
					+ " was added at (" + x + ", " + y + ") to change from "
					+ oldHeatHere + " to " + heatHere + ".");
		}
		return this;
	} // / addHeat()
	 ...
	 * extreme (COLD or HOT).
	 * <p>
	 * Note that a HeatSpace has a wrap-around geography: the south-eastern
	 * neighbor of the cell at (x, y) is the cell at ((x + 1) % getSizeX (), (y
	 * - 1) % getSizeY ()). Of course, <tt>getSizeX()</tt> and
	 * <tt>getSizeY()</tt> are inherited from Diffuse2dImpl.
	 * 
	 * @param type
	 * 		(in) HeatSpace.COLD or HeatSpace.HOT
	 * @param targetCell
	 * 		(inout) the Point at the center of the 9-cell neighborhood; we
	 * 		change it to a Point randomly selected from among those with the
	...
	public int findExtremeType$X$Y(int type, Point targetCell, Grid2d world) {
		...
		// Prime the loop by assuming that the extreme heat is right at
		// targetCell:
		...
		for (y = targetCell.y - 1; y <= targetCell.y + 1; y++) {
			for (x = targetCell.x - 1; x <= targetCell.x + 1; x++) {
				...
				boolean candidateIsBetter = (type == COLD) ? (candidateHeat < bestHeat)
		...
		// Choose a point at random from the list of Points tied for most
		// extreme:
		int offset = Globals.env.uniformIntRand.getIntegerWithMin$withMax(0,
				(heatList.size() - 1));
		Point bestCell = (Point) heatList.get(offset);

		// We've found the requested extreme. Set the (inout) variable
		// targetCell,
	...
	/**
	 * Did you think you could override Diffuse2d.stepRule()? You can -- as we
	 * have done here -- but the method will never be invoked.
	 * <p>
	 * Diffuse2d inherits from Ca2d, which inherits from DblBuffer2d, which
	 * maintains two lattices (old and new). As the Swarm Reference Guide states
	 * for DblBuffer2d, <tt>putValue()</tt>, which <tt>stepRule()</tt>
	 * presumably invokes, is "overridden so writes happen to newLattice".
	 */
